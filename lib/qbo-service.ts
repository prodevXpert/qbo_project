...
import { QBOTokens, QBOCustomer, QBOVendor, QBOBill, QBOInvoice, QBOAttachable, UploadedFile } from './types'
...
export class QBOService {
  ...
  private async retryWithBackoff<T>(
    ...
    try {
      ...
      if (error.fault?.error?.[0]?.code === '3200' && retryCount < this.maxRetries) {
  ...
  async createCustomer(displayName: string, parentRef?: string): Promise<QBOCustomer> {
    return this.retryWithBackoff(async () => {
      return new Promise((resolve, reject) => {
  ...
  async createVendor(displayName: string): Promise<QBOVendor> {
    return this.retryWithBackoff(async () => {
      return new Promise((resolve, reject) => {
        this.qbo.createVendor({ DisplayName: displayName }, (err: any, result: any) => {
          if (err) reject(err)
          else resolve(result)
        })
      })
  ...
  async uploadAttachment(
    ...
    return this.retryWithBackoff(async () => {
      return new Promise((resolve, reject) => {
        ...
        this.qbo.createAttachable(attachable, file.data, (err: any, result: any) => {
          if (err) reject(err)
          else resolve(result)
        })
      })
  ...
  async createInvoiceFromBillableExpenses(
    ...
    return this.retryWithBackoff(async () => {
      return new Promise(async (resolve, reject) => {
        try {
          // Get custom field ID for Point of Contact
          const customFieldId = await this.getOrCreateCustomField('Point of Contact')
