...
import { parseDate, validateAmount, generateIdempotencyKey } from './utils'
...
export class CSVProcessor {
  ...
  validateRow(row: CSVRow, rowIndex: number): ValidationError[] {
    ...
    if (!row.ProjectName?.trim()) {
      errors.push({ row: rowIndex, field: 'ProjectName', message: 'Project name is required' })
    }
    if (!row.CustomerName?.trim()) {
      errors.push({ row: rowIndex, field: 'CustomerName', message: 'Customer name is required' })
    }
    if (!row.VendorName?.trim()) {
      errors.push({ row: rowIndex, field: 'VendorName', message: 'Vendor name is required' })
    }

    // Date validation
    const billDate = parseDate(row.BillDate, this.settings.strictDateParsing)
    if (!billDate) {
      errors.push({ row: rowIndex, field: 'BillDate', message: 'Invalid bill date format' })
    }

    const invoiceDate = parseDate(row.InvoiceDate, this.settings.strictDateParsing)
    if (!invoiceDate) {
      errors.push({ row: rowIndex, field: 'InvoiceDate', message: 'Invalid invoice date format' })
    }

    // Amount validation
    const amount = validateAmount(row.BillLineAmount)
    if (amount === null) {
      errors.push({ row: rowIndex, field: 'BillLineAmount', message: 'Invalid amount format' })
    }

    // Currency validation
    const currency = row.Currency?.trim() || this.settings.defaultCurrency
    if (!/^[A-Z]{3}$/.test(currency)) {
      errors.push({ row: rowIndex, field: 'Currency', message: 'Invalid currency code (must be 3-letter ISO code)' })
    }
  ...
  async dryRun(rows: CSVRow[]): Promise<DryRunResult[]> {
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      actions.push(`Find or create Sub-Customer (Project): "${row.ProjectName}" under "${row.CustomerName}"`)
      actions.push(`Find or create Vendor: "${row.VendorName}"`)
      actions.push(`Create Bill for ${row.BillLineAmount} ${row.Currency || this.settings.defaultCurrency}`)
      
      const attachments = row.AttachmentFiles?.split(';').filter(f => f.trim()) || []
      if (attachments.length > 0) {
        actions.push(`Attach ${attachments.length} file(s) to Bill: ${attachments.join(', ')}`)
        if (this.settings.alsoAttachToInvoice) {
          actions.push(`Also attach files to Invoice`)
        }
      }
      
      actions.push(`Create Invoice from billable expenses with PO# ${row.PONumber}`)
  ...
  async processRow(
    ...
    try {
      ...
      let subCustomer = await this.qboService.findCustomerByName(row.ProjectName)
      if (!subCustomer) {
        subCustomer = await this.qboService.createCustomer(row.ProjectName, customer.Id)
      }
      ...
      const fileNames = row.AttachmentFiles?.split(';').filter(f => f.trim()) || []
      
      for (const fileName of fileNames) {
        const file = attachments.get(fileName.trim())
        if (file) {
          try {
            const attachable = await this.qboService.uploadAttachment(file, 'Bill', bill.Id)
      ...
      if (this.settings.alsoAttachToInvoice) {
        for (const fileName of fileNames) {
          ...
          if (file) {
            try {
              await this.qboService.uploadAttachment(file, 'Invoice', invoice.Id)
            } catch (error) {
